        ...
        if (RestrictedSecurity.isFIPSSupportPKCS12()) {
            if (!useNativePBES2) {
                throw new RuntimeException(
                        ...
                        "the PKCS12 keystore is not needed.");
            }
                ...
                if (nativeCrypto.PBES2Derive(password, password.length, salt, salt.length, iterCount, hashIndex, keyLength, key) != -1) {
                    ...
                    if (nativeCrypto.FIPSMode() != 0) {
                        ...
                        throw new RuntimeException(
                            ...
                            "Please enable FIPS module in the configuration. ");
                    }
                } else if (nativeCryptTrace) {
                    throw new RuntimeException(
                        "Native PBE derive failed for algorithm " + hashAlgo + ". Please check if the OpenSSL support the PBKDF2 key derive");
                }
            } else if (nativeCryptTrace) {
                throw new RuntimeException(
                    "The algorithm " + hashAlgo + " is not supported in native code.");
            }
        }
        try {
